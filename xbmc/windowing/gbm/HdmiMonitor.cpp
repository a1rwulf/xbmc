...
void CHdmiMonitor::FDEventCallback(int id, int fd, short revents, void *data)
{
  ...
  while ((device = udev_monitor_receive_device(udevMonitor)) != NULL)
  {
    ...
    CLog::Log(LOGERROR, "CHdmiMonitor - Action %s (\"%s\", \"%s\")", action,
                                                                            udev_device_get_syspath(device),
                                                                            udev_device_get_devpath(device));

    //CApplicationMessenger::GetInstance().PostMsg(TMSG_RENDERER_REINIT);
    const char* status = udev_device_get_sysattr_value(device, "card0-DP-1/status");
    CLog::Log(LOGERROR, "CHdmiMonitor - HDMI Status: %s", status);

    if (!strcmp(status, "connected"))
    {
      hdmiMonitor->GetWinSystem().SetHdmiState(HdmiState::CONNECTED);
    }
    else
    {
      hdmiMonitor->GetWinSystem().SetHdmiState(HdmiState::DISCONNECTED);
    }
