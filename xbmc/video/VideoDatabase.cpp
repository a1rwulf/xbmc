...
bool CVideoDatabase::LinkMovieToTvshow(int idMovie, int idShow, bool bRemove)
{
   try
  {
    ...
    if (m_cdb.getDB()->query_one<CODBMovie>(odb::query<CODBMovie>::idMovie == idMovie))
    {
      ...
      {
        if (show.load() && static_cast<int>(show->m_idTVShow) == idShow)
          return true;
      }
...
int CVideoDatabase::GetPlayerPlayCount(unsigned long fileId)
{
  try
  {
    std::shared_ptr<odb::transaction> odb_transaction (m_cdb.getTransaction());
    ...
    if (!m_cdb.getDB()->query_one<CODBPlayCount>(odb::query<CODBPlayCount>::file->idFile == fileId
  ...
  try
  {
    ...
    if(m_cdb.getDB()->query_one<CODBFile>(query::idFile == idFile, odbFile))
    {
      ...
      */
      ...
      details.SetPlayCount(std::max(details.GetPlayCount(), static_cast<int>(GetPlayerPlayCount(odbFile.m_idFile)))); //TODO: Replace by signed int?
    ...
    const std::map<std::string, std::string> &artwork, int idMovie /* = -1 */)
{
  try
  {
    ...
    if (!details.m_set.title.empty())
    {
      ...
      if (!m_cdb.getDB()->query_one<CODBSet>(odb::query<CODBSet>::name == details.m_set.title, *set))
      {
        set->m_name = details.m_set.title;
...
void CVideoDatabase::SetMovieDetailsValues(CODBMovie& odbMovie, CVideoInfoTag& details)
{
  ...
  odbMovie.m_credits = StringUtils::Join(details.m_writingCredits, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_videoItemSeparator);
...
void CVideoDatabase::DeleteResumeBookMark(const CFileItem& item)
{
  ...
  try
  {
    ...
    gVideoDatabaseCache.clearCacheByFileID(fileID);
  }
  catch (std::exception& e)
  {
    CLog::Log(LOGERROR, "%s exception - %s", __FUNCTION__, e.what());
...
void CVideoDatabase::AddBookMarkToFile(const std::string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    gVideoDatabaseCache.clearCacheByFileID(idFile);
  }
  catch (std::exception& e)
  {
    CLog::Log(LOGERROR, "%s exception - %s", __FUNCTION__, e.what());
    ...
    if (mediaType == VIDEODB_CONTENT_MOVIES)
    {
      ...
      for (odb::result<ODBView_Movie>::iterator i = resMovies.begin(); i != resMovies.end(); i++)
      {
        ...
        for (std::vector<odb::lazy_shared_ptr<CODBTag> >::iterator tag = i->movie->m_tags.begin(); tag != i->movie->m_tags.end(); tag++)
        {
          ...
          if (static_cast<int>((*tag)->m_idTag) == idTag)
          {
...
bool CVideoDatabase::GetStreamDetails(CVideoInfoTag& tag)
{
  ...
  std::shared_ptr<CStreamDetails> cached = gVideoDatabaseCache.getStreamDetails(tag.m_iFileId);
  if (cached)
  {
    details = *cached;
  ...
  std::shared_ptr<CStreamDetails> detailsPtr(new CStreamDetails);
  *detailsPtr = details;
  gVideoDatabaseCache.addStreamDetails(tag.m_iFileId, detailsPtr);
...
bool CVideoDatabase::GetArtForItem(int mediaId, const MediaType &mediaType, std::map<std::string, std::string> &art)
{
  try
  {
    std::shared_ptr<std::map<std::string, std::string> > cached = gVideoDatabaseCache.getArtMap(mediaId, mediaType);
    if (cached)
    {
      art = *cached;
      return true;
...
bool CVideoDatabase::GetPlayCounts(const std::string &strPath, CFileItemList &items)
{
  ...
  try
  {
    ...
    for (odb::result<CODBFile>::iterator i = res.begin(); i != res.end(); i++)
    {
      ...
      if (item)
      {
        item->GetVideoInfoTag()->SetPlayCount(GetPlayerPlayCount(i->m_idFile));
...
int CVideoDatabase::GetPlayCount(int iFileId)
{
  ...
  try
  {
    ...
    return GetPlayerPlayCount(odb_file.m_idFile);
  }
  catch (std::exception& e)
  {
    CLog::Log(LOGERROR, "%s exception - %s", __FUNCTION__, e.what());
...
bool CVideoDatabase::GetActorsNav(const std::string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    ...
    for (const auto &i : mapItems)
    {
      CFileItemPtr cached = gVideoDatabaseCache.getPerson(i.first);
      if (cached)
      {
        items.Add(cached);
        continue;
      ...
      gVideoDatabaseCache.addPerson(i.first, pItem);
    }
...
void CVideoDatabase::AdjustQueryFromUrlOptions(odb::query<ODBView_Movie>& movie_query, CVideoDbUrl& videoDbUrl)
{
...
void CVideoDatabase::GetTranslatedString(unsigned long id, std::string& var, std::string key1, std::string key2, uint64_t updatedAt)
{
  ...
  std::string newVar = gVideoDatabaseCache.getTranslation(key, updatedAt);
  if (!newVar.empty())
  {
    var = newVar;
  }
