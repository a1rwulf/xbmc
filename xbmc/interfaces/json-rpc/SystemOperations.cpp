...
JSONRPC_STATUS CSystemOperations::Screenshot(const std::string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
  std::string *data(new std::string());
  CApplicationMessenger::GetInstance().SendMsg(TMSG_BASE64_SCREENSHOT, -1, -1, static_cast<void*>(data));
  result["data"] = *data;
  delete data;
